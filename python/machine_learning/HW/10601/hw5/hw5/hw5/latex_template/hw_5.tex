...
\newcommand{\dueDate}{Thursday, October 27 at 11:59 PM}
...
\begin{subparts}
    ...
    \textbf{Hint 2:} After substituting in your expressions for $\frac{\partial \ell}{\partial \hat{y}_l}$ and $\frac{\partial \hat{y}_l}{\partial b_k}$, try to rearrange terms so that you encounter the expression $\hat{y}_k \sum_l y_l$. What is the value of $\sum_l y_l$?
...
\begin{lstlisting}[language=Shell]
python3 neuralnet.py tiny_train.csv tiny_validation.csv \
...
\begin{algorithm}
      \begin{algorithmic}[1]
        \Procedure{Forward}{$\av$}
          ...
          \State Cache intermediate value(s) for the backward pass \Comment{See Written Question 1.2(c)}
