...
\newcommand{\dueDate}{Thursday, Oct 13 at 11:59 PM}
...
\begin{notebox}
\paragraph{Summary} In this assignment, you will build a sentiment polarity analyzer, which will be capable of analyzing the overall sentiment polarity (positive or negative) for movie reviews using logistic regression. In the written component, you will study linear and logistic regression.
\end{notebox}
...
\part[1] We require the feature values to be in the range $[0,1]$ to generate an image. Using the setup outlined in \ref{q:aa}, propose a different procedure subject to this constraint that does not require a gradient calculation.
...
\part[2] \sall Now let's consider whether logistic regression is well-suited for this task. Suppose the exact same white narwhal in a dark ocean background was used to generate the training set. The training photos were captured with the side view of the narwhal centered in the photo at a distance of between 30-50 meters from the camera. Which (if any) of the below descriptions of a \textbf{test image} would the model predict as ``narwhal''?
...
The following questions should be completed as you work through the programming component of this assignment. \textbf{Please ensure that all plots are computer-generated}. For all the questions below, unless otherwise specified, use the constant learning rate 0.001.
...
{\bf Embeddings }
\texttt{word2vec.txt} contains the \emph{word2vec} embeddings of \texttt{15k} words. Not every word in each review is present in the provided \texttt{word2vec.txt} file. We treat such missing words as ``out-of-vocabulary'' and ignore them. Each line consists of a word and its embedding separated by tabs:
...
\begin{enumerate}
    \item First, we would like to exclude words from the review that are not included in the word2vec dictionary. Let  $\xv\_\text{trim}^{(i)} = \text{TRIM}(\xv^{(i)})$, where $\text{TRIM}(\xv^{(i)})$ trims the list of words $\xv^{(i)}$ by only including words of $\xv^{(i)}$ present in \texttt{word2vec.txt}.
    ...
    $$\boldsymbol{\phi}\left(\xv^{(i)}\right) = \frac{1}{J} \sum_{j=1}^J word2vec(\xv\_\text{trim}^{(i)}_j) $$
...
\begin{itemize}
    ...
    \item \textbf{word2vec embeddings} to use for the word embedding feature extraction methods.
\end{itemize}
...
All of your reported numbers should be within 0.00001 of the reference solution, and you should round the error values to 6 decimal places. The following example is the reference solution for the small dataset after 500 training epochs with learning rate 0.001.
\begin{lstlisting}
...
\begin{figure}[H]
        \centering
        \includegraphics[width = 0.7\textwidth]{Pipeline_v4.png}
...
\begin{enumerate}
    ...
    \item \texttt{<feature\_dictionary\_input>}: path to the word2vec feature dictionary \texttt{.txt} file (see Section~\ref{featuremodels})
...
\begin{enumerate}
    ...
    \item \texttt{<learning\_rate>}: float specifying the learning rate; in the reference output, we set the learning rate to be $0.001$ for all datasets
\end{enumerate}
