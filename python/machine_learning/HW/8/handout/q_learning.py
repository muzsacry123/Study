...
def Q(W: np.ndarray, state: np.ndarray, 
    ...
    '''
    # TODO: Implement this!
    # # Add bias to the state vector
    # state = np.concatenate(([1], state))
    
    # Compute the Q-value for the given action
    if action is not None:
        q = np.dot(W[action], state)
        return round(q, 5)
    
    # Compute Q-values for all actions
    else:
        q_all = np.dot(W, state)
        return np.round(q_all, 5)
...
if __name__ == "__main__":
    ...
    if env_type == "mc":
        ...
        env = GridWorld(mode, True) # TODO: Replace me!
    else: 
    ...
    for episode in range(episodes):
        ...
        for iteration in range(max_iterations):
            ...
            # TODO: Select an action based on the state via the epsilon-greedy strategy.
            q = Q(W, state)
            rand = np.random.randint(1, 1001)
            
            if (rand / 1000) <= epsilon:
                action = np.random.randint(0, env.action_space - 1)
            else:
                action = np.argmax(q)
